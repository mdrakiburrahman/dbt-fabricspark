...
- Fixed contract enforcement aborting with `'dict' object has no attribute '__name__'` on any model with an `ARRAY`, `MAP` or `STRUCT` column. Livy reports primitive column types as strings (`string`, `decimal(10,2)`) but complex ones as nested JSON schema dicts (`{"type": "array", "elementType": "string", "containsNull": true}`), and `data_type_code_to_name` assumed the string form, so `{{ config(contract={'enforced': true}) }}` failed on the first complex column. Complex types are now rendered into their full SQL DDL string — `array<string>`, `map<string,int>`, `struct<a:string,b:int>`, and arbitrarily nested combinations such as `array<struct<sku:string,qty:int>>`. Rendering the whole structure rather than just the top-level kind keeps contracts honest: a model producing `array<string>` against a contract declaring `array<int>` is still a `data type mismatch`, whereas collapsing both to `ARRAY` would have made every array interchangeable. Both sides of the comparison (the model SQL and the yaml-derived empty-schema query) route through the same function, so the check stays symmetric. Unrecognized shapes now degrade to a readable string instead of raising, so no cursor description can crash the adapter again. Affects both the high-concurrency and singleton backends. ([#255](https://github.com/microsoft/dbt-fabricspark/issues/255))

- Made `spark_config` verifiable instead of a black box. Previously there was no way to find out what the adapter actually sent: the session-create payload was never logged (the high-concurrency path logged only the `sessionTag`), `spark_config` was excluded from `_connection_keys()` so `dbt debug` did not echo it, and the three code paths disagreed about what they forwarded — the high-concurrency path copied a fixed 16-key allowlist and dropped the rest silently, the singleton Fabric path POSTed the whole dict verbatim, and local mode dropped everything except `kind`, so `spark_config.conf` never reached local Livy at all. When a session setting appeared not to take effect, you could not tell whether the adapter sent it, whether the service ignored it, or whether it was overridden downstream. Now: the full payload is logged at DEBUG in every mode, `spark_config` and `high_concurrency` both appear in `dbt debug`, and all three paths forward `spark_config` verbatim. The adapter-derived `sessionTag` still wins over a user-supplied one because it controls high-concurrency session packing, but it now warns when overriding instead of doing it silently. Note that a Fabric resource profile is applied *after* the session `conf` and will override keys it defines — this is Fabric precedence, not an adapter bug, and the new payload log makes it diagnosable. ([#257](https://github.com/microsoft/dbt-fabricspark/issues/257))
...
- Fixed `materialized_lake_view` models failing intermittently with `MLV_ARTIFACT_NOT_FOUND` ("references a lakehouse that no longer exists or can't be resolved") when an on-demand refresh runs immediately after the MLV is created. Fabric's item metadata is eventually consistent, so the refresh job can outrun the `CREATE MATERIALIZED LAKE VIEW` that preceded it by seconds. This is the same race the adapter already retried for `MLV_NOT_FOUND` and `MLV_LINEAGE_NOT_FOUND`, but the substring check missed the `MLV_ARTIFACT_NOT_FOUND` variant, and Fabric reports it with `isRetriable: false`, so nothing else retried it either and the run failed outright. All three not-found codes are now handled together as one transient class, retried with the existing jittered exponential backoff and still bounded by `statement_timeout`. Genuinely permanent failures are unaffected and still raise on the first attempt.
